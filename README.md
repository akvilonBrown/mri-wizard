# mri-wizard
This repository contains models that were involved to start and develop grain segmentaion project, as well as postprocessing scripts and some benchmarks.
The demo data and the main segmentation model should be downloaded separately, if required.

The folder xp_pilot_bench contains 2D Unet segmentation scripts for demo training and inference processing slices of 3D NMR images, in the subfolders created by Hydra configuration tool there is a result of training and checkpoints.
The folder xp_stacked_model_bench contains 2.5D Unet segmentation scripts that process stacks of slices.
...
Checkpoints of the trained model should be downloaded and unpacked in the folder specified as RESULTS_FOLDER in environment variables. 
...
The data is created for the demo data that can be downloaded and unpacked after cloning this repository on the local computer.
