...
def main(cfg : DictConfig) -> None:
    ...
    model = models.UNet3(n_channels=input_dim, n_classes=5, bilinear=False).to(device)
    ...
    log.info(f"smean {smean}, var_limit {var_limit}, clip_min {clip_min}, clip_max {clip_max}") 
    ...
    dataset_training = MultiDataSet(inputs=strain,
                                            targets=ttrain,
                                            transform=transforms_training,
                                            depth = frame_depth)
    
    #strain, ttrain, sval, tval, stest, ttest
    dataset_validation = MultiDataSet(inputs=sval, 
                                            targets=tval,
    ...
    fig = inference.plot_training(training_losses, validation_losses, validation_losses, gaussian=True, sigma=1, figsize=(10, 4))
    plt.savefig(cfg.train.savefig_png)
    plt.savefig(cfg.train.savefig_pdf)
    plt.plot(lr_rates)
    plt.savefig(cfg.train.savefig_png2)
    plt.savefig(cfg.train.savefig_pdf2)   
